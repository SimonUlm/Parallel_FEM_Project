...
namespace Mesh {
    ...
    void GlobalMesh::Scatter(LocalMesh &local_mesh, Skeleton::Skeleton &skeleton) {
        ...
        // MPI stuff
        MPI_Comm comm_ = skeleton.get_comm();
        int rank_ = skeleton.get_rank();
        long nnodes;
        if (rank_ == 0)
            nnodes = nodes_.count();
        MPI_Bcast(&nnodes, 1, MPI_LONG, 0, comm_);
        ...
        if (rank_ == 0) {
            for (int process = 1; process < m_ * n_; ++process) {
                local_mesh.m_ = m_;
                local_mesh.n_ = n_;
                TransferGlobalToLocal(local_mesh, global_nodes_priority, comm_, process);
                if (process != 1)
                ...
                MPI_Isend(mesh_data.data_(), 7,
                          MPI_LONG, process, 0, comm_, &request);
                MPI_Isend(mesh_sender.nodes_.data_(), (int) mesh_sender.nodes_.count() * 2,
                         MPI_DOUBLE, process, 0, comm_, &request);
                MPI_Isend(mesh_sender.elements_.data_(), (int) mesh_sender.elements_.count() * 7,
                         MPI_LONG, process, 0, comm_, &request);
                MPI_Isend(mesh_sender.boundary_.data_(), (int) mesh_sender.boundary_.count() * 4,
                         MPI_LONG, process, 0, comm_, &request);
                MPI_Isend(mesh_sender.local_to_global.data_(), (int) mesh_sender.local_to_global.count(),
                          MPI_LONG, process, 0, comm_, &request);
            }
            TransferGlobalToLocal(local_mesh, global_nodes_priority, comm_, rank_);
        } else {
            MPI_Recv(mesh_data.data_(), 7, MPI_LONG, 0, 0, comm_, &status);
            local_mesh = LocalMesh(mesh_data);
            MPI_Recv(local_mesh.nodes_.data_(), (int) local_mesh.nodes_.count() * 2,
                      MPI_DOUBLE, 0, 0, comm_, &status);
            MPI_Recv(local_mesh.elements_.data_(), (int) local_mesh.elements_.count() * 7,
                      MPI_LONG, 0, 0, comm_, &status);
            MPI_Recv(local_mesh.boundary_.data_(), (int) local_mesh.boundary_.count() * 4,
                      MPI_LONG, 0, 0, comm_, &status);
            MPI_Recv(local_mesh.local_to_global.data_(), (int) local_mesh.local_to_global.count(),
                      MPI_LONG, 0, 0, comm_, &status);
        }
        MPI_Bcast(global_nodes_priority.data_(), (int) global_nodes_priority.count(), MPI_LONG, 0, comm_);
    ...
    void GlobalMesh::TransferGlobalToLocal(LocalMesh &local_mesh, Util::Vector<long> &global_nodes_priority,
        ...
        for (auto &boundary_edge : boundary_)
            if (edge_flags[boundary_edge.m_])
                ++bdry_count;
        ...
        if (bdry_count != 0) {
            ...
            for (auto &boundary_edge : boundary_) {
                if (edge_flags[boundary_edge.m_]) {
                    ...
                    local_mesh.boundary_(bdry_count).m_ = global_to_local_edges[boundary_edge.m_];
                    ++bdry_count;
                }
            }
        }
