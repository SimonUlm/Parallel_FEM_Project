...
namespace Util {
    ...
    void VectorConverter::AccumulatedToDistributed(Vector<double> &local_vector) const {
    ...
    void VectorConverter::DistributedToAccumulated(Vector<double> &local_vector, MPI_Comm comm) const {
        assert(local_vector.count() == local_to_global_->count());

        // Create global vector from local vector
        Vector<double> global_vector_send(n_global_nodes_);
        for (long i = 0; i < local_vector.count(); ++i)
            global_vector_send((*local_to_global_)(i)) = local_vector(i);

        // Allreduce
        Vector<double> global_vector_recv(n_global_nodes_);
